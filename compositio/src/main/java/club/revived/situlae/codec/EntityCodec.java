package club.revived.situlae.codec;
...
import club.revived.situlae.model.annotation.Identifier;
import club.revived.situlae.model.annotation.Ignore;
import club.revived.situlae.model.annotation.Property;
...
public final class EntityCodec<T extends Entity> implements Codec<T> {
  ...
  private RecordComponent findComponent(@NotNull final String bsonName) {
    ...
    for (final RecordComponent component : components) {
      if (component.isAnnotationPresent(Property.class)) {
        if (component.getAnnotation(Property.class).value().equals(bsonName)) {
          return component;
        }
      }
    }
  ...
  private String getFieldName(final RecordComponent component) {
    ...
    if (component.isAnnotationPresent(Property.class)) {
      return component.getAnnotation(Property.class).value();
    }
