...
public final class ChatHistoryCommand {
    ...
    private void handleLogs(CommandSender sender, String targetName, String timeframe, String query, int page, List<ChatMessage> logs) {
        if (logs.isEmpty()) {
        ...
        sender.sendRichMessage("<gold>Chat History for <yellow>" + targetName + " <gray>(" + timeframe + ") (Page " + page + "/" + totalPages + "):");
        ...
        for (int i = start; i < end; i++) {
            ...
            sender.sendRichMessage("<gray>[" + formattedDate + "] " + (log.isBlocked() ? "<red>[BLOCKED] " : "") + "<white>" + log.getContent());
        }
    }
}
